...
function getRowItems(row: Row<page>) {
  return [
  checkPermission(Actions.UPDATE_PAGE) && {
      type: "label",
      label: "ACTIONS :",
    },
    checkPermission(Actions.UPDATE_PAGE) && {
    ...
    checkPermission(Actions.DELETE_PAGE) && {
      label: "Delete",
      icon: "i-lucide-trash",
      color: "error",
      async onSelect() {
...
const loadData = async () => {
  strUri.value = `/api1/pages?pages=${pages.value}&limit=${limit.value}&search=${searchKeywords.value}`;
  await axios
    .get(strUri.value, {
      headers: {
    ...
    .then(async (res) => {
      ...
      data.value = await res.data.data.pageapis;
    })
...
onMounted(async () => {
  ...
  if (checkPermission(Actions.CREATE_PAGE)) {
    ...
    limit.value = 6;
    setItems(["pages"]);
    // strUri.value = `/api1/pages?pages=${pages.value}&limit=${limit.value}`;
    await loadData();
  } else {
    setItems([]);
