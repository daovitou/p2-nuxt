...
import Createbookmodal from "../../books/createbookmodal.vue";
import { useInfiniteScroll } from "@vueuse/core";
import Deletebookmodal from "../../books/deletebookmodal.vue";
import Editbookmodal from "../../books/editbookmodal.vue";
...
onMounted(() => {
  if (!checkPermission(Actions.READ_BOOK_CATEGORY)) {
    ...
    strUri.value = `/api1/books?cat=${id}&pages=${page.value}&limit=${limit.value}`;
  }
});
const fetchMoreItems = async () => {
  ...
  const response = await axios.get(
    `/api1/books?cat=${id}&pages=${page.value}&limit=${limit.value}&search=${searchKeywords.value}`,
    {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
