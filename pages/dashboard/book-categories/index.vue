...
import Createbookcategorymodal from "./createbookcategorymodal.vue";
import Editbookcategorymodal from "./editbookcategorymodal.vue";
import Deletebookcategorymodal from "./deletebookcategorymodal.vue";
...
const data = ref<bookcateogiry[]>([]);
const pageData = ref<pageDataType>();

const columns: TableColumn<bookcateogiry>[] = [
  {
    accessorKey: "id",
    header: "#",
...
function getRowItems(row: Row<bookcateogiry>) {
  return [
  checkPermission(Actions.UPDATE_BOOK_CATEGORY) && {
      type: "label",
      label: "ACTIONS :",
    },
    checkPermission(Actions.UPDATE_BOOK_CATEGORY) && {
...
function openCreate() {
  modal.open(Createbookcategorymodal, {
    onSuccess() {
      modal.reset();
      toast.add({
...
function openEdit() {
  modal.open(Editbookcategorymodal, {
    id: userId.value,
    onSuccess() {
      modal.reset();
      toast.add({
...
function openDelete() {
  modal.open(Deletebookcategorymodal, {
    id: userId.value,
    onSuccess() {
      modal.reset();
      toast.add({
...
const loadData = async () => {
  strUri.value = `/api1/book-categories?pages=${pages.value}&limit=${limit.value}&search=${searchKeywords.value}`;
  await axios
    .get(strUri.value, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
...
onMounted(async () => {
  ...
  if (checkPermission(Actions.READ_BOOK_CATEGORY)) {
    isLoading.value = true;
    pages.value = 1;
    limit.value = 6;
    setItems(["book categories"]);
    strUri.value = `/api1/book-categories?pages=${pages.value}&limit=${limit.value}`;
    await loadData();
  } else {
    setItems([]);
  ...
  <div data-aos="fade-left">
    ...
    >
      ...
      <UButton
        ...
        label="Create"
        @click="openCreate()"
        v-if="checkPermission(Actions.CREATE_BOOK_CATEGORY)"
      />
    </div>
    <UTable
      :loading="isLoading"
    ...
    >
      <template #name-cell="{ row }">
        ...
        <router-link
          class="capitalize"
          :to="`/dashboard/book-categories/${row.original.id}`"
          @click="
            () => {
              setItems(['book categories', row.original.name]);
            }
          "
          >{{ row.original.name }} =
