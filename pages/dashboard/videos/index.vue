...
const fetchMoreItems = async () => {
  ...
  strUri.value =
    Number(catid) & (Number(catid) > 0)
      ? `/api1/videos?cat=${catid}&pages=${page.value}&limit=${limit.value}&search=${searchKeywords.value}`
      : `/api1/videos?pages=${page.value}&limit=${limit.value}&search=${searchKeywords.value}`;
...
onMounted(() => {
  if (!checkPermission(Actions.READ_VIDEO)) {
    setItems([]);
